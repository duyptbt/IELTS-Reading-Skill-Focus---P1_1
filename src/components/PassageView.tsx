...
export const PassageView: React.FC<PassageViewProps> = ({
  ...
  const handleMouseUp = () => {
    ...
    // If eraser is selected in the top bar, remove highlights that match selected text
    if (highlighterColor === 'eraser') {
      const matchingHls = highlights.filter(
        (h) => h.paragraphId === pId && (h.text.toLowerCase().includes(text.toLowerCase()) || text.toLowerCase().includes(h.text.toLowerCase()))
      );
    ...
    // If a highlighter color is already selected in the top bar, apply immediately
    if (highlighterColor) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-5 sm:p-8 space-y-6">
        ...
        {/* Paragraphs */}
        <div
          className="space-y-6 text-slate-800 font-serif leading-relaxed"
        ...
        >
          {PARAGRAPHS.map((p) => (
            <div
              ...
              className="p-3.5 sm:p-4 rounded-lg transition-all duration-200 hover:bg-slate-50/80 border border-transparent hover:border-slate-200/80 group"
            >
              <p className="text-slate-800 tracking-normal text-justify leading-relaxed">
